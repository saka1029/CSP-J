...
import jp.saka1029.cspj.problem.ConstraintEncoder;
...
public class MinisatSolver implements Solver {
    ...
    private void encode(Variable<?> variable, Bind bind) {
        ...
        if (variable instanceof Constraint<?>) {
            ...
            c.encodeVariable(domain, bind, new ConstraintEncoder() {
	...
	void result(jp.saka1029.minisatj.core.Solver solver, Result result, VecLit deny) {
...
//            Log.info(bind);
        if (debug.contains(Debug.Reduced)) {
        	Log.info("SatSolver: **** reduced problem ****");
        	for (Variable<?> v : problem.variables())
        		Log.info("SatSolver: %s : %s", v, bind.get(v));
        }
