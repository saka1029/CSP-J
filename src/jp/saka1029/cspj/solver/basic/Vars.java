...
public class Vars<T> {
	...
	Vars(int n, Domain<T> domain) {
		for (int i = 0; i < n; ++i)
			variables.add(problem.variable(null, domain));
	}
	
	@SafeVarargs
	Vars(Domain<T>... domains) {
		for (Domain<T> d :domains)
			variables.add(problem.variable(null, d));
	}
	
	public static <T> Vars<T> of(int n, Domain<T> domain) {
		return new Vars<>(n, domain);
	}

	@SafeVarargs
	public static <T> Vars<T> of(Domain<T>... domains) {
		return new Vars<>(domains);
	}

	public Vars<T> constraint(ConstraintPredicate<T> predicate) {
